...
![I thought the "if stats programs were cars" meme needed tweaking... R是瘋狂麥斯裡面的車隊，只是上面站的是一群統計學家跟寫程式的人...](image/rmeme.jpg){width="384"}
...
R是必須要安裝的，否則沒辦法使用。RStudio則是R語言的"整合開發環境' (Integrated Development Environment, IDE)，許多小功能讓你寫程式更舒服。雖說 RStudio 不是必須，但是本書程式碼全部都是在RStudio中寫出來的。可以從[這裡](https://joe11051105.gitbooks.io/r_basic/content/environment_settings/RStudio_introduction.html)有更詳細的RStudio環境介紹。


## 第一個 R 程式碼

複製下面程式碼，輸入 R console，按下 Enter；或是在 RStudio 中開啟的script，輸入後將輸入游標在同一行按 Ctrl+Enter。
