...
-   {car}的`ncvTest`檢定變異數同值，若顯著代表變異數不同值。
...
-   Barlett's test：顯著代表變異數不同值
-   Levene's test：顯著代表變異數不同值。
...
進行隨機設計(complete design, CRD)、完全隨機區集設計(random complete
block design,
RCBD)等試驗設計時，其實是將觀測值放進一個線性模式裡面來分析。
...
我習慣用`anova`，因為可以用`data.frame`轉成表格，方便輸出成csv檔
...
test的縮寫，是農業領域研究習慣使用的事後檢定(post-hoc test)方法。

LSD利用ANOVA的MSE(誤差均方)作為$\sigma^2$，兩兩之間進行的t檢定。當平均值差超過LSD值時，代表兩平均值之間具有顯著差異。
...
當因子之間交感作用顯著時，不能單純比較變因等級之間(V1與V2)的平均值，應該要做因子組合之間的交感比較。
...
以上`riceYield`為例，要先產生敘述統計表，再把LSD結果與其合併，所幸`LDS.test`回傳的結果已包含這些資料
```{r}
...
上述資料分析，可以寫成一個函數`dlsd`，`dlsd`的意思是用一行函數從讀取`data`到`LSD`分析結果。
