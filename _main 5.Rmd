...
<<<<<<< HEAD
以下引用[Advanced R的Why R](https://adv-r.hadley.nz/introduction.html)說明使用R的好理由:

-   R完全免費而且開源。

-   R的社群相當多元而且樂於討論。

-   R有大量的統計分析、機器學習、數據視覺化、資料整理套件，任何想做的分析，幾乎已有人寫好程式碼與教學。
...
<<<<<<< HEAD
![如果統計程式是一台車](image/rmeme.jpg){width="384"}

=======
![I thought the "if stats programs were cars" meme needed tweaking... R是瘋狂麥斯裡面的車隊，只是上面站的是一群統計學家跟寫程式的人...](%5Cimage%5CRmeme.jfif){width="384"}
>>>>>>> 755c67710231c06fd78df9d19e08012ed38e61cb

[from reddit](https://www.reddit.com/r/rstats/comments/dn7fa7/i_thought_the_if_stats_programs_were_cars_meme/)
...
R是必須要安裝的，否則沒辦法使用。RStudio則是R語言的"整合開發環境' (Integrated Development Environment, IDE)。[這裡](https://joe11051105.gitbooks.io/r_basic/content/environment_settings/RStudio_introduction.html)有更詳細的RStudio環境介紹。
=======
下載與安裝[R](https://cran.r-project.org/) 及 [RStudio](https://posit.co/download/rstudio-desktop/)

-   R是必須要安裝的，否則沒辦法使用。

-   RStudio則是R語言的"整合開發環境' (Integrated Development Environment, IDE)，許多小功能讓你寫程式更舒服。雖說 RStudio 不是必須，但是本書程式碼全部都是在RStudio中寫出來的，可見 RStudio 有奪好用。

可以看[這個](https://joe11051105.gitbooks.io/r_basic/content/environment_settings/RStudio_introduction.html)，更詳細的RStudio環境介紹
>>>>>>> 755c67710231c06fd78df9d19e08012ed38e61cb
...
<<<<<<< HEAD
複製下面程式碼，貼上至R console，按下Enter；或是在 RStudio 中開啟新的script，貼上程式碼將輸入游標在同一行按Ctrl+Enter。

這是你的第一個R程式碼，嗨，世界!
=======
複製下面程式碼，輸入 R console，按下 Enter；或是在 RStudio 中放入
...
<<<<<<< HEAD
以下章節都會包含程式碼 (上方灰框) 與執行結果 (下方灰框)，可以直接複製程式碼，貼上在 R 介面中執行。
...
你會發現，R很像是Google助理，輸入一行指令，執行後就會返回一行結果，這種一來一往式的程式語言叫做直譯語言 (Interpreted Language)。但是，一段 R 的程式碼也有可能很多行，譬如下面畫圖的函數，就包含許多行程式碼。 　 　
>>>>>>> 755c67710231c06fd78df9d19e08012ed38e61cb
...
<<<<<<< HEAD
上面是利用內建資料`iris`畫成的圖，只要學會使用`ggplot2`套件，不同類型的資料也能畫出好看又專業的圖。
=======
上面是利用R內建的資料`iris`所進行的繪圖，只要學會使用`ggplot2`套件，不同類型的資料也都可以畫出好看又專業的圖。
...
=======
過去大多數人是從R 的基礎入門，缺點是很難看懂。

現在許多R的教程會直接建議從{tidyverse}開始學(我也是)，譬如以下這篇訪問，RStudio創辦人 J.J. Allaire(簡稱JJ)接受 Joseph B. Rickert(簡稱JBR)[訪問](https://cosx.org/2016/11/interview-j-j-allaire/)也提到這一點。
...
我買過R跟Python的參考書，但我認為R語言奠基在一個不斷進化的基礎上，而且最好的R語言參考書正是以Gitbook電子書的形式免費發行在網路上供人閱讀，因此花錢買書並不太划算。
...
有結果後就可以準備輸出，這時可以用R忍者[謝益輝](https://yihui.org/cn/vitae/)開發的套件，例如：
...
另外是RMarkdown的另一種替代方案[Quarto](https://quarto.org/)，可以用來產生簡潔易懂的程式碼與報告。


D4S第二版也有專文介紹[Quarto](https://r4ds.hadley.nz/quarto.html)。
...
我買過R跟Python的參考書，但我認為R語言奠基在一個不斷進化的基礎上，而且最好的R語言參考書正是以Gitbook電子書的形式免費發行在網路上供人取用，所以購買書本並從中學習其實不太好。這邊提供一些有名的參考書。


-   [R for Data Science](https://r4ds.had.co.nz/index.html) by Hadley Wickham and Garrett Grolemund。 [Hadley Wickham](https://hadley.nz/)是[RStudio](https://posit.co/download/rstudio-desktop/)的工程師，基本上也是R語言的耶穌。他讓R語言變得更容易使用，這本書介紹R資料科學，也是我的第一本教科書。

![R4DS](%5Cimage%5CR4DS.png){width="200"}

-   [R for Data Science 第二版](https://r4ds.hadley.nz/) 好書的第二版一定也是好書(🦐挺起來)。

-   [Advanced R](https://adv-r.hadley.nz/introduction.html) 當你有一段三行以上的程式碼要一再複製貼上重複使用，就要考慮使用函數，這本書介紹除了函數編程(Functional programming)以外也介紹R的物件導向編程(Object-oriented Programming)

![Advanced R](%5Cimage%5CADVR.png){width="200"}
...
有結果後就可以準備輸出，由[R忍者謝益輝](https://yihui.org/cn/vitae/)撰寫的幾個套件就好用，例如：

-   [RMarkdown cookbook](https://bookdown.org/yihui/rmarkdown-cookbook/)

![RMC](%5Cimage%5CRMC.png){width="200"}

-   [bookdown: Authoring Books and Technical Documents with R Markdown](https://bookdown.org/yihui/bookdown/)

![BDC](%5Cimage%5CBDC.jpg){width="200"}
...
上方印出的東西稱為資料表 (dataframe)，資料表由2個變數 (variables) `處理`和`產量`組成。變數名稱所在的位置稱為欄位 (columns)，欄位是有順序的，在上表中，第1欄為`處理`、第2欄為`產量`。每個欄位下面都有資料值 (values)，稱為觀測值 (observations)，觀測值左方的數字為列 (row) 數，可以看到這筆資料有2欄10列。
...
箭頭 (\<-) 在 R 中是代表賦值 (assign) 的操作子 (operator)，意思是把"箭頭右方"的物件，放入"箭頭左方"的物件。
...
=======
然而，多重比較並不是具有目的性的探討研究結果， （林俊隆老師：先進行變方分析，再進行平均值之間的多重比較幾乎成了統計分析的八股）
>>>>>>> 755c67710231c06fd78df9d19e08012ed38e61cb


在 R 中，計畫性的比較，被歸納為一般線性假說(General Linear Hypotheses)和模型參數多重比較(multiple comparisons for parametric models)，可以利用 {multcomp} 套件的`glht`或是{lsmeans}的進行。
...
-   因為變方分析顯示品種間具顯著差異，所以進行的LSD檢定屬於「受到變方分析結果保護的**保護的LSD** (protected LSD)，第一型錯誤率屬於「以試驗為單位的第一型錯誤率」。
...
線性對比的檢定方式有兩種，利用{multcomp}的`glht`函數，或是
{lsmeans}的`contrast`函數
...
-   virginica和setosa的花萼長度之間*具顯著差異*
-   virginica的花萼長度*非*setosa的1.5倍
...
> 'Tukey'亦可置換為"Dunnett", "Tukey", "Sequen", "AVE", "Changepoint", "Williams", "Marcus", "McDermott", "UmbrellaWilliams", "GrandMean"其一
...
具有交感項的多重比較，可用{emeans}套件的`emmeans`和`contrast`，{emeans}是{lsmeans}的更新版本。

先用`emmeans`估算各效應，再利用`contrast`計算各項線性對比。
...
e_wt <- emmeans( m2, ~ wool*tension)

e_wt
```

`contrast`函數很強大，除了產生線性對比以外，也可以探討正交的多項式對比效應(orthogonal polynomial contrasts)、對控制組的對比等
...
# 多項式對比效應
# emmeans:::poly.emmc(1:6) 可以查看多項式對比效應的係數
contrast(e_wt, "poly")

# 成對比較
contrast(e_wt, "pairwise")

# 對控制組的比較
contrast(e_wt, "trt.vs.ctrl")

# 自訂的線性對比，假設想進行 LA - 2MA + HA -100  的線性對比
# method放線性對比，offset放常數項
contrast(e_wt,
         method=list(
           'LA - 2MA + HA' = c(1,0,2,0,1,0),
           'LA - 2MA + HA - 100' =c(1,0,2,0,1,0)),
         offset = c(0,-100))
=======
glht(m1, 
     linfct = mcp( Species = 
